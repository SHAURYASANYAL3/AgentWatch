...
def _ev(
    ...
    *,
    ...
    cmd_keys = ("command", "cmd", "shell", "exec")
    _raw_cmd = next(
        (
            v
            for k in cmd_keys
            if isinstance(_args.get(k), str) and _args[k].strip()
            for v in [_args[k]]
        ),
        None,
    )
    return AgentEvent(
        ...
        status=status,
        step_number=step,
        tool_call=ToolCallData(tool_name=tool, arguments=_args, raw_command=_raw_cmd)
        if tool
        else None,
    )
